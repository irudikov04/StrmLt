...
with tab2:
    ...
    if st.button("Рассчитать влияние изменений", type="primary", key="calculate_impact"):
        ...
        # Расчет общего влияния на винрейт
        total_impact = (
            health_change_pct * weights["Здоровье"] * 0.3 +  # Здоровье влияет умеренно
            armor_change_abs * 10 * weights["Броня"] * 0.25 +  # Броня сильно влияет
            damage_change_pct * weights["Урон"] * 0.4 +  # Урон сильно влияет на урон-ориентированных
            attack_speed_change_pct * weights["Скорость атаки"] * 0.35 +  # Скорость атаки умеренно влияет
            mana_change_pct * weights["Мана"] * 0.2  # Мана слабо влияет для не-магов
        )
        ...
        with col_res2:
            ...
            # Сложность внедрения
            complexity = "Простая" if max_change_pct < 10 else "Средняя" if max_change_pct < 20 else "Сложная"
            st.metric("Сложность внедрения", complexity)
        ...
        # Визуализация весов параметров для этого героя
        st.subheader("📈 Влияние параметров на винрейт (для данного героя)")
        df_importance = pd.DataFrame(list(weights.items()), 
                                     columns=["Параметр", "Вес влияния"])
        fig3 = px.bar(df_importance.sort_values(by="Вес влияния"), 
                     x="Вес влияния", y="Параметр", 
                     orientation='h', color="Вес влияния",
                     color_continuous_scale="Viridis")
        st.plotly_chart(fig3, use_container_width=True)
        
        # Детальная таблица изменений
        st.subheader("📋 Детализация изменений")
        changes_data = {
            "Параметр": ["Здоровье (HP)", "Броня", "Урон", "Скорость атаки", "Мана (MP)"],
            "Текущее": [
                f"{current_params['Здоровье']}",
                f"{current_params['Броня']}",
                f"{current_params['Урон']}",
                f"{current_params['Скорость атаки']}",
                f"{current_params['Мана']}"
            ],
            "Новое": [
                f"{proposed_params['Здоровье']}",
                f"{proposed_params['Броня']:.1f}",
                f"{proposed_params['Урон']}",
                f"{proposed_params['Скорость атаки']}",
                f"{proposed_params['Мана']}"
            ],
            "Абс. изменение": [
                f"{proposed_params['Здоровье'] - current_params['Здоровье']:+.0f}",
                f"{proposed_params['Броня'] - current_params['Броня']:+.1f}",
                f"{proposed_params['Урон'] - current_params['Урон']:+.0f}",
                f"{proposed_params['Скорость атаки'] - current_params['Скорость атаки']:+.0f}",
                f"{proposed_params['Мана'] - current_params['Мана']:+.0f}"
            ],
            "% изменение": [
                f"{health_change_pct:+.1f}%" if current_params['Здоровье'] > 0 else "0%",
                f"{(armor_change_abs/current_params['Броня'])*100 if current_params['Броня'] != 0 else '∞':+.1f}%",
                f"{damage_change_pct:+.1f}%" if current_params['Урон'] > 0 else "0%",
                f"{attack_speed_change_pct:+.1f}%" if current_params['Скорость атаки'] > 0 else "0%",
                f"{mana_change_pct:+.1f}%" if current_params['Мана'] > 0 else "0%"
            ]
        }
        
        # Создаем таблицу с цветовым кодированием
        ...
        if delta > 8:
            ...
            st.markdown(f"""
            Герой **{hero}** станет слишком сильным (+{delta:.1f}% к винрейту).
            
            **Рекомендуемые корректировки:**
            1. Уменьшите урон на 10-15%
            2. Снизьте скорость атаки на 10-20%
            3. Увеличьте стоимость способностей на 20-30%
            4. Рассмотрите уменьшение здоровья на 5-10%
            """)
        elif delta > 4:
            st.warning("### ⚠️ **УМЕРЕННЫЙ РИСК УСИЛЕНИЯ**")
            st.markdown(f"""
            Герой **{hero}** может стать сильнее оптимального (+{delta:.1f}% к винрейту).
            
            **Рекомендуемые корректировки:**
            1. Небольшое уменьшение брони или здоровья
            2. Балансировка через стоимость предметов
            3. Увеличение задержки (cooldown) на ключевые способности
            """)
        elif delta < -8:
            st.error("### ⚠️ **ВЫСОКИЙ РИСК ОСЛАБЛЕНИЯ**")
            st.markdown(f"""
            Герой **{hero}** станет слишком слабым ({delta:.1f}% к винрейту).
            
            **Рекомендуемые корректировки:**
            1. Увеличьте базовое здоровье на 15-20%
            2. Добавьте пассивную регенерацию маны/здоровья
            3. Уменьшите стоимость способностей на 20-30%
            4. Увеличьте базовый урон на 10-15%
            """)
        elif delta < -4:
            st.warning("### ⚠️ **УМЕРЕННЫЙ РИСК ОСЛАБЛЕНИЯ**")
            st.markdown(f"""
            Герой **{hero}** может стать слабее оптимального ({delta:.1f}% к винрейту).
            
            **Рекомендуемые корректировки:**
            1. Увеличение урона на 5-10%
            2. Улучшение базовых характеристик
            3. Уменьшение задержки (cooldown) на второстепенные способности
            """)
        else:
            st.success("### ✅ **БЕЗОПАСНЫЙ ДИАПАЗОН**")
            st.markdown(f"""
            Предлагаемые изменения находятся в безопасном диапазоне (±{abs(delta):.1f}% к винрейту).
            
            **Следующие шаги:**
            1. Протестировать изменения на тестовом сервере
            2. Собрать отзывы от профессиональных игроков
            3. Мониторить статистику в течение 2-4 недель
            4. При необходимости внести микро-корректировки
            """)
        ...
        if st.button("💾 Сохранить предложенные изменения", key="save_changes"):
            ...
            with st.expander("Просмотреть сохраненные настройки"):
                st.write(f"**Герой:** {hero}")
                st.write(f"**Прогнозируемый винрейт:** {new_winrate:.1f}%")
...
with tab3:
    ...
    if st.button("Сгенерировать предмет", type="primary"):
        ...
        # Демо-характеристики
        characteristics = {
            "Урон": random.randint(10, 50),
            "Защита": random.randint(0, 15),
            "Редкость": random.choice(["Обычный", "Редкий", "Эпический", "Легендарный"]),
            "Стоимость": random.randint(2000, 7000)
        }
